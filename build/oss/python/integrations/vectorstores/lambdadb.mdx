...
if "LAMBDADB_PROJECT_API_KEY" not in os.environ:
    os.environ["LAMBDADB_PROJECT_API_KEY"] = getpass.getpass("Enter your LambdaDB project API key: ")

if "LAMBDADB_BASE_URL" not in os.environ:
    os.environ["LAMBDADB_BASE_URL"] = getpass.getpass("Enter your LambdaDB base URL: ")

if "LAMBDADB_PROJECT_NAME" not in os.environ:
    os.environ["LAMBDADB_PROJECT_NAME"] = getpass.getpass("Enter your LambdaDB project name: ")
```
...
- `collection_name`: Name of a collection in LambdaDB (required)
...
- `index_configs`: Index configurations for the collection (default: None)
- `partition_configs`: Partition configurations for the collection (default: None)
- `create_if_not_exists`: Whether to create the collection if it does not exist (default: True)
...
LambdaDB supports filtering using [query string syntax](https://docs.lambdadb.ai/guides/search/query-string):
...
For detailed documentation of all `LambdaDBVectorStore` features and configurations, head to the [API reference](https://pypi.org/project/langchain-lambdadb/).
